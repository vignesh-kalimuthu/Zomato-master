...
const NavSm = () => {
    ...
    const [allTypes, setAllTypes] = useState([
     ...
     const {type} = useParams();
         return (
        <>
          <div className="lg:hidden flex bg-white shadow-lg p-3 fixed bottom-0 z-10 w-full  items-center justify-between md:justify-evenly text-gray-500 border">
            {allTypes.map((items) => (
              <Link to={`/${items.id}`}>
             
               <div className={ type === items.id ? "flex flex-col relative items-center text-4xl text-zomato-400" : "flex flex-col items-center text-3xl "}>
                  
                 <div className={type === items.id && "absolute -top-3 w-full h-2 border-t-2   border-zomato-300"}/>
                     {items.icon}
